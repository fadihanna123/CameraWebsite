...
export const doRegister = async (req: Request, res: Response) => {
  ...
  if (!uname || !email || !mobnr || !psw || !repsw) {
    ...
    storeError('Du måste fylla i alla obligatoriska rutor!', 'POST', '/register');
    ...
    if (!validator.isEmail(email)) {
      ...
      storeError('Du måste fylla in en korrekt e-postadress!', 'POST', '/register');
      res.json({ message: 'Du måste fylla in en korrekt e-postadress!' });
    } else {
      // Om e-postadressen är korrekt skriven.
      if (!validator.isLength(psw, { min: 8 }) && !validator.isLength(repsw, { min: 8 })) {
        // Om lösenord och bekräfta lösenord fälten inte innehåller starka lösenord som har minst 8 tecken.
        storeError('Du måste välja ett lösenord som är minst 8 tecken!', 'POST', '/register');
        ...
        if (psw !== repsw) {
          ...
          storeError('Ditt lösenord matchar inte det bekräftade lösenordet.', 'POST', '/register');
          ...
          if (findUser !== null) {
            ...
            storeLog('Du är redan registrerad hos oss. Du kan logga in ovan.', 'POST', '/register');
            ...
            try {
              const file = (req as any).files.myFile ? (req as any).files.myFile : '';
              ...
              await file.mv('./uploads/' + file.name, file.name, (err: Error) => {
                if (err) {
              ...
              if (userModel) {
                storeLog('Tack för registrering. \n Var vänlig och logga in.', 'POST', '/register');
                res.send({
